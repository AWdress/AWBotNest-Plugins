...
| 插件 | id | 触发 | scope | 说明 |
|------|----|------|-------|------|
| 聊天互动助手 | `keyword_auto_reply` | 监听群消息 | user | 无需关键词即可按概率回复，支持追加回复、逐规则冷却、限群、自动删除、模板变量 |
...
| PT站自动签到 | `pt_multi_checkin` | 定时 / 立即签到（平台 Cookie） | standalone | Vue 管理的 24 站签到中心；HTTP 优先，遇到 CF/雷池或动态页面自动降级 CloakBrowser |
