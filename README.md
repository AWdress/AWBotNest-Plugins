...
- **获取群组名称**：Vue 界面需要显示群组名称而非数字 ID 时，可在后端通过 `httpx` 调用 `http://localhost:18001/api/chats/{chat_id}`（需管理员 Bearer 令牌），响应 `{id, title, type}`。查询失败时应回退显示 ID。
...
| 憨憨赠豆 | `hhan_bonus` | `.hh` `.hhs`（平台 Cookie） | user | 从平台 Cookie 同步读取 HHCLUB 登录态，支持单人/批量赠送憨豆与持久化冷却 |
| 憨憨转盘 | `hhan_lottery` | 配置页操作（平台 Cookie） | user | 在配置页设置次数并开始/停止抽奖，完成后推送结果，同时保存最近结果与累计统计 |
| 憨憨一键已读 | `hhan_read` | Vue 配置页操作（平台 Cookie） | user | 扫描 HHanClub 站内信，仅批量提交未读消息，支持停止、进度、Cookie 检查与运行历史 |
...
| 插件 | id | 触发 | scope | 说明 |
|------|----|------|-------|------|
| 趣味答题 | `quiz_game` | 群发「开启答题」 | user | 出题抢答，答对自动发魔力奖励，支持连胜加成、答题黑名单（AI/天行出题） |
| 数字炸弹 | `bomb_game` | 群内开启后参与 | both | 回复+金额组奖池，轮流猜数字，爆炸者出局，中奖按比例分池 |
| 发红包 | `red_packet_send` | 群里发包 | user | 用你的账号发拼手气红包，群友回复参与，自动分配发放魔力 |
| 幸运抽奖 | `human_lottery` | 群内自然命令 | user | 用用户账号发起抽奖，群友发送关键词参与，到时随机开奖并自动发奖；支持提前开奖、取消、随机延迟和历史管理 |
...
| 自动抢红包 | `red_packet_grab` | 监听正文及图片口令红包 / Vue 配置界面 | user | 支持正文口令、动态图片财富密码、OCR 验证码及复制已确认口令兜底；可限制发包人/群组，含抢包记录 |
