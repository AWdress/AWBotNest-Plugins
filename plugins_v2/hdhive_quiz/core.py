...
async def _safe_notify(ctx, client, text, level="info"):
    try:
        ...
        await ctx.notify(rows or [{"项目": "详情", "内容": "暂无内容"}],
                         level=level, category="影巢答题红包", account=client)
    except Exception:  # noqa: BLE001
        pass
...
async def _resolve_answer(ctx, parsed: dict) -> tuple[object, str, str]:
    ...
    if cfg.get("llm_enabled", False):
        ans, err = await _engine.ask_answer(ctx, q, opts, qtype, ctx.log)
        if ans:
            if qtype == "judge":
...
async def setup(ctx):
    ...
    async def _api_test_llm(req):
        ...
        ans, err = await _engine.ask_answer(ctx, test_q, test_opts, "single", ctx.log)
        if err:
