...
__plugin__ = {'name': 'Emby 工具箱',
 ...
 'changelog': 'v2.0.8 修复电影被误扫为剧集\n- 删除单集 Genre 与季名刮削严格只处理 Series，电影不再请求季列表\n- 季列表请求限定 Season 类型，404 与非 JSON 响应安全跳过且不再输出乱码或带密钥 URL\n- 修复国家/语言标签任务的扫描计数初始化\n\nv2.0.7 适配平台统一 Cron 编辑器\n- 定时维护字段声明标准 cron 格式并使用宿主 CronInput\n- 调度改用 schedule_cron 正式接口并完整支持范围、步长和列表表达式\n\nv2.0.6 补齐密钥显示按钮\n- Emby 与 TMDB API Key 默认隐藏并提供独立眼睛按钮\n- 配置页读取脱敏值时通过平台受控接口取得真实密钥\n\n'
