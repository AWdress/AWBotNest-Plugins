    ...
    "config_schema": {
        ...
        "rules_text": {
            ...
            "fields": {
                ...
                "extra_reply_enabled": {"type": "boolean", "label": "发送追加回复", "default": False},
                "extra_reply": {"type": "string", "label": "追加回复内容", "default": "叮！恭喜你喜提特等奖掉落。掉落 {number} 茉莉"},
                "cooldown_notify": {
...
_storage_state: dict = {}
_storage_tasks: set = set()


def _persist(ctx, key, value):
    _storage_state[key] = value
    task = ctx.create_task(ctx.storage.set(key, value), name=f"keyword-reply-storage:{key}")
    _storage_tasks.add(task)
    task.add_done_callback(_storage_tasks.discard)


def _parse_rules(raw, *, default_match: str = "contains", default_cooldown: float = 24, default_midnight: bool = False) -> list[tuple[str, str, bool, str, float, str, bool, bool, float, list[str], float, bool, str]]:
    """解析规则；旧配置自动继承原来的全局匹配方式与冷却时间。"""
    rules: list[tuple[str, str, bool, str, float, str, bool, bool, float, list[str], float, bool, str]] = []
        ...
        for d in raw:
            if isinstance(d, dict):
                ...
                if reply:
                    ...
                    extra_reply_enabled = bool(d.get("extra_reply_enabled", False))
                    extra_reply = str(d.get("extra_reply", "叮！恭喜你喜提特等奖掉落。掉落 {number} 茉莉") or "").strip()
                    rules.append((keyword, reply, bool(d.get("cooldown_notify", True)), match_type, cooldown, trigger_mode, reset_at_midnight, count_for_leaderboard, fun_reply_chance, fun_replies, trigger_chance, extra_reply_enabled, extra_reply))
        return rules
    for line in str(raw or "").splitlines():
        ...
        if keyword and reply:
            rules.append((keyword, reply, True, default_match, default_cooldown, "any", default_midnight, True, 0.0, [], 100.0, False, ""))
    return rules
...
async def _is_reply_to_me(message, own_id: int) -> bool:
    """当前消息是否回复了本账号发出的消息。"""
    replied = await message.get_reply_message()
    return bool(replied and (getattr(replied, "out", False) or replied.sender_id == own_id))
...
def _save_cooldowns(ctx) -> None:
    ...
    _persist(ctx, _COOLDOWNS_KV_KEY, rows)


def _restore_cooldowns(ctx) -> None:
    """恢复仍在当前冷却窗口内的记录，并顺便清理过期或损坏数据。"""
    _user_cooldowns.clear()
    raw = _storage_state.get(_COOLDOWNS_KV_KEY, []) or []
    if not isinstance(raw, list):
        _persist(ctx, _COOLDOWNS_KV_KEY, [])
        return
...
async def _refresh_leaderboard_names(ctx, client, account: str, chat_id: int) -> None:
    ...
    rows = _storage_state.get(_LEADERBOARD_KV_KEY, []) or []
    ...
    if not targets:
        return
    try:
        users = [await client.get_entity(int(item["user_id"])) for item in targets]
        ...
        for item in targets:
            ...
            if user and item.get("name") != _display_name(user):
                ...
                changed = True
        if changed:
            _persist(ctx, _LEADERBOARD_KV_KEY, rows)
...
def _render(reply: str, sender=None, *, random_values: list[int] | None = None) -> str:
    ...
    if sender:
        uid = sender.id
        uname = sender.first_name or sender.username or str(uid)
        for ch in ("\\", "_", "*", "[", "]", "(", ")", "~", "`", ">", "#", "+", "-", "=", "|", "{", "}", ".", "!"):
...
def _choose_reply(standard_reply: str, fun_reply_chance: float, fun_replies: list[str]) -> tuple[str, bool]:
    """按百分比选择回复，并返回是否命中了趣味文案。"""
    if fun_replies and fun_reply_chance > 0 and random.random() * 100 < fun_reply_chance:
        return random.choice(fun_replies), True
...
def _schedule_delete(ctx, message, delay: int):
    ...
    task = ctx.create_task(_runner(), name="关键词回复自动删除")
...
def _schedule_delete_rich(ctx, client, chat_id: int, sent_at: float, delay: int):
    ...
    async def _runner():
        try:
            await asyncio.sleep(delay)
            async for item in client.iter_messages(chat_id, limit=30):
    ...
    task = ctx.create_task(_runner(), name="羊毛榜富文本自动删除")
    _pending_tasks.add(task)
    task.add_done_callback(_pending_tasks.discard)
    ...
    async def keyword_listener(event):
        ...
        if leaderboard_command and text.strip() == leaderboard_command:
            ...
            sent = await client.send_message(
                chat_id, _leaderboard_text(ctx, account_id, chat_id, limit), reply_to=message.id,
            )
        ...
        # 屏蔽名单用户的消息不触发
        if sender and sender.id in blacklist:
            return
        try:
        ...
        try:
            for keyword, reply, cooldown_notify, match_type, cooldown_hours, trigger_mode, reset_at_midnight, count_for_leaderboard, fun_reply_chance, fun_replies, trigger_chance, extra_reply_enabled, extra_reply in rules:
                if not _match(text, keyword, match_type):
                    continue
                if trigger_mode == "reply_to_me" and not await _is_reply_to_me(message, own_id):
                    continue
                    ...
                    if time.time() - last_time < cooldown_secs:
                        if cooldown_notify:
                            ...
                            cd_msg = await client.send_message(
                                chat_id, cd_text, reply_to=message.id
                            )
                ...
                selected_reply, used_fun_reply = _choose_reply(reply, fun_reply_chance, fun_replies)
                random_values: list[int] = []
                sent = await client.send_message(
                    chat_id, _render(selected_reply, sender, random_values=random_values),
                    reply_to=message.id
                )
                if count_for_leaderboard:
                    _record_welfare(ctx, account_id, chat_id, sender, keyword)
                _schedule_delete(ctx, sent, delete_after)
                if extra_reply_enabled and extra_reply and not used_fun_reply:
                    rendered_extra = extra_reply.replace(
                        "{number}", str(random_values[0]) if random_values else ""
                    )
                    extra_sent = await client.send_message(
                        chat_id, _render(rendered_extra, sender), reply_to=message.id
                    )
                    _schedule_delete(ctx, extra_sent, delete_after)
