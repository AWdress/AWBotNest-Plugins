      ...
      <div v-show="tab === 'settings'" class="layout">
        ...
        <div class="detail">
          <template v-if="group === 'cred'">
            ...
            <section class="card">
              <label class="row"><span>Cookie</span><div class="secret-field"><input v-model="cfg.cookie" class="inp" :type="secretVisible.cookie ? 'text' : 'password'" placeholder="F12 里 zhuque.in 请求的整条 Cookie" /><button type="button" aria-label="显示或隐藏 Cookie" @click="secretVisible.cookie = !secretVisible.cookie"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12Z"/><circle cx="12" cy="12" r="3"/></svg></button></div></label>
              <label class="row"><span>X-Csrf-Token</span><div class="secret-field"><input v-model="cfg.xcsrf" class="inp" :type="secretVisible.xcsrf ? 'text' : 'password'" placeholder="F12 请求头 x-csrf-token" /><button type="button" aria-label="显示或隐藏 X-Csrf-Token" @click="secretVisible.xcsrf = !secretVisible.xcsrf"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12Z"/><circle cx="12" cy="12" r="3"/></svg></button></div></label>
