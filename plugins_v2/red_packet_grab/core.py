...
__plugin__ = {
    "name": "自动抢红包", "id": "red_packet_grab", "version": "1.2.4",
...
async def setup(ctx):
    ...
    _records = Records(ctx, state)
    ctx.add_cleanup(_records.close)
    ...
    def _sender_allowed(message, sender, chat_id, cfg):
    ...
    async def on_packet(event):
        if not event.is_group or not (event.photo or event.document):
            return
        ...
        if rotating_password and not packet_has_remaining(caption):
            ctx.log.info(
                ...
                getattr(chat, "title", None) or event.chat_id,
                event.chat_id, message.id,
            )
            return

        sender_name = _sender_allowed(message, sender, event.chat_id, cfg)
        if sender_name is None:
            return
        try:
            await _grabber.handle_new_packet(
                client, message, sender, chat, sender_name=sender_name,
    ...
    @ctx.on_edited_message()
    @ctx.on_message(incoming=True)
    async def on_plaintext_packet(event):
        ...
        try:
            await _grabber.handle_plaintext_packet(
                client, message, sender, chat, sender_name=sender_name, command=command,
                join_delay=to_float(cfg.get("join_delay", 2)),
                notify=cfg.get("notify_owner", True),
                ttl_secs=max(1, _to_int(cfg.get("activity_ttl_minutes", 30), 30)) * 60,
            )
    ...
    async def on_reply(event):
        if not event.is_group or not event.message.reply_to_msg_id:
            return
        client, message = event.client, event.message
        sender, chat = await event.get_sender(), await event.get_chat()
        cfg = _effective_cfg(ctx)
