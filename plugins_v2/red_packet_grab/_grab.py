...
class Grabber:
    ...
    async def handle_plaintext_packet(
        self, client, message, sender, chat, sender_name: str, command: str,
    ...
    async def handle_new_packet(
        self, client, message, sender, chat, sender_name: str, join_delay: float,
    ...
    async def handle_group_text(self, client, message, sender, chat, min_len: int, max_len: int) -> None:
        group_id = message.chat_id
        chat_label = self._remember_chat(chat)
        # 该群没有进行中的红包就不缓存
        if not any(g == group_id for (g, _p) in self._active):
            return
        me = getattr(client, "me", None)
        if me and sender and sender.id == me.id:
            return  # 不缓存自己发的
        ...
        if not re.fullmatch(r"[0-9A-Za-z]+", text):
            return
        reply_to_id = int(getattr(message, "reply_to_msg_id", 0) or 0)
    ...
    async def handle_reply(
        self, client, message, sender, chat, success_markers: list[str], transfer_prefix: str,
        ...
        if candidate:
            ...
            packet_hint = self._packet_hint_from_reply(replied, group_id)
        if not code:
            self._log.debug(
            ...
            return
        confirmer_id = sender.id if sender else None
    ...
    def _packet_hint_from_reply(self, replied, group_id: int) -> int:
        parent_id = int(getattr(replied, "reply_to_msg_id", 0) or 0) if replied else 0
    ...
    async def _read_replied_candidate(
        ...
        try:
            ...
            self._log.debug(
                ...
                message.chat_id, message.reply_to_msg_id, exc,
            )
            return ""
