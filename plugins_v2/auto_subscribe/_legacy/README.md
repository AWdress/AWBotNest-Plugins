...
| 奈飞 | Tudum Top10 公开 TSV | 全球榜 + 94 国国家榜；**富元数据模式默认开**（多几次请求，补年份/干净剧名，识别最准；TSV 无年份） |
| 猫眼 | 猫眼专业版票房/网播 | 用平台 `ctx.browser`(CloakBrowser/Playwright) 预取 Cookie，取不到时降级无 Cookie；个别榜可能受风控为空 |
...
`render_mode: "vue"`，配置弹窗由自带的 `frontend/src/Config.vue` 渲染（模块联邦），宽屏 master-detail 布局，用**容器查询**按弹窗实际宽度自适应（窄时分组栏收为顶部横排、单列铺满）。页签栏右侧带版本徽章，便于确认加载的前端构建。三个页签：
...
- **平台 AI 辅助识别**：可选开启。仅当常规搜索无法识别榜单条目时，调用平台 AI 提取标准片名、电影/剧集类型和季号；结果仍需经 NextFind 核验，默认关闭。
...
- `ai_assist_recognition`：是否在常规搜索无结果时调用平台 AI 辅助识别，默认关闭。
...
- 识别靠 `/search` 标题+年份+类型搜索（无豆瓣id/番组id精确定位），同名不同作品可能误配；奈飞富元数据模式（默认开）补年份可显著缓解，关闭则只有剧名、匹配变差。
- 猫眼 Cookie 经平台 `ctx.browser` 预取（首次会触发下载浏览器内核）；取不到时降级无 Cookie，网播接口可能返回受限。
