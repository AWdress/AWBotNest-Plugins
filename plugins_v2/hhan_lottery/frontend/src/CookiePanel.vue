  ...
  <section class="cookie-panel">
    ...
    <template v-else>
      ...
      <section class="card" :class="{ inactive: cfg.cookie_source !== 'manual' }">
        ...
        <div class="secret-field"><input v-model="cfg.manual_cookie" :type="cookieVisible ? 'text' : 'password'" autocomplete="off" spellcheck="false" placeholder="name=value; name2=value2" :disabled="cfg.cookie_source !== 'manual'"><button type="button" :disabled="!hasManual" :aria-label="cookieVisible ? '隐藏 Cookie' : '显示 Cookie'" @click="cookieVisible = !cookieVisible"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12Z"/><circle cx="12" cy="12" r="3"/></svg></button></div>
...
.card { padding:19px; border:1px solid var(--line); border-radius:14px; background:#111c2b; } .card.inactive { opacity:.65; } h3 { margin:0 0 4px; font-size:14px; } .text-danger { border:0; color:#ff929d; background:transparent; cursor:pointer; } .text-danger:disabled { opacity:.4; cursor:not-allowed; } .secret-field{position:relative;margin-top:15px}.secret-field input{width:100%;box-sizing:border-box;padding:11px 48px 11px 12px;border:1px solid #354a64;border-radius:9px;color:#edf5fe;background:#0c1623;font:13px ui-monospace,Consolas,monospace}.secret-field input:focus{outline:2px solid #378cf0;outline-offset:2px}.secret-field button{position:absolute;right:4px;top:50%;transform:translateY(-50%);width:36px;height:36px;display:grid;place-items:center;border:0;border-radius:8px;background:transparent;color:#91a3b9;cursor:pointer}.secret-field button:hover:not(:disabled){color:#3d94f7;background:#142b48}.secret-field button:disabled{opacity:.4;cursor:not-allowed}.secret-field svg{width:18px;fill:none;stroke:currentColor;stroke-width:1.8}.security { margin:10px 0 0; color:#bd9d68; font-size:11px; }
