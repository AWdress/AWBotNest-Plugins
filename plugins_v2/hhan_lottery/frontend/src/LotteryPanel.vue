      ...
      <div class="card settings">
        ...
        <div class="settings-columns">
          <section class="setting-group">
            ...
            <label v-if="cfg.scheduled_stop_enabled" class="stacked-field"><span>停止日期时间</span><input v-model="cfg.scheduled_stop_at" type="datetime-local"></label>
            <p v-if="cfg.scheduled_stop_enabled" class="mode-note">计划会持久化，平台重启后继续剩余任务，并按时停止。</p>
          </section>

          <section class="setting-group">
            <h4>大奖与通知</h4>
            <label class="feature-toggle"><span><b>实时大奖通知</b><small>命中后立即推送富文本表格</small></span><input v-model="cfg.prize_notify_enabled" type="checkbox"></label>
            <div v-if="cfg.prize_notify_enabled" class="stop-box prize-notice-box">
            ...
            <label class="feature-toggle"><span><b>命中大奖后自动停止</b><small>关闭时只通知，不中断任务</small></span><input v-model="cfg.stop_on_prize" type="checkbox"></label>
            ...
            <div class="notification-options">
              <label class="check"><input v-model="cfg.notify_result" type="checkbox"> 完成结果</label>
              <label class="check"><input v-model="cfg.notify_cookie_error" type="checkbox"> Cookie 异常</label>
            </div>
          </section>
        </div>
...
.stop-box { margin:6px 0 12px; padding:12px; border:1px solid #2e425b; border-radius:9px; background:#0d1827; } .prize-options{display:flex;flex-wrap:wrap;gap:4px 16px;padding-bottom:8px;border-bottom:1px solid #26384f}.stop-box .check { margin:3px 0; }.settings .stop-box label:not(.check,.toggle-row,.feature-toggle){grid-template-columns:1fr;gap:6px;margin:11px 0}.notification-options{display:flex;flex-wrap:wrap;gap:4px 20px;margin-top:12px;padding-top:10px;border-top:1px solid var(--line)} input[type=text] { width:100%; box-sizing:border-box; padding:9px 10px; border:1px solid #344861; border-radius:8px; color:#edf4fc; background:#0d1725; font:inherit; }
...
@media(max-width:700px){ header{display:block}.state{margin-top:13px}.live-stats,.settings-columns{grid-template-columns:1fr}.live-stats ul{grid-template-columns:1fr}.setting-group+ .setting-group{padding:18px 0 0;border-left:0;border-top:1px solid var(--line)}.card-heading{display:grid}.toggle-row{width:100%}.settings label:not(.check,.toggle-row,.feature-toggle){grid-template-columns:1fr}.result{min-height:0} footer button{flex:1} }
</style>
