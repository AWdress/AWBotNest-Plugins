...
class RecordStore:
    ...
    def _persist(self, key, value):
        ...
        task = self._ctx.create_task(self._kv.set(key, value), name=f"transfer-storage:{key}")
    ...
    def sites_with_data(self) -> list[str]:
        """返回所有已有聚合数据的站点名。"""
        return [k[len(_AGG_PREFIX):] for k in self._state if k.startswith(_AGG_PREFIX)]

    def reset_site(self, site: str) -> None:
        key = _AGG_PREFIX + site
        self._state.pop(key, None)
        task = self._ctx.create_task(self._kv.delete(key), name=f"transfer-storage-delete:{key}")
        self._tasks.add(task); task.add_done_callback(self._tasks.discard)
    ...
    def _load_agg(self, site: str) -> dict:
        raw = self._state.get(_AGG_PREFIX + site)
        if not raw:
            return {}
    ...
    def _save_agg(self, site: str, agg: dict) -> None:
        ...
        try:
            self._persist(_AGG_PREFIX + site, agg)
        except Exception:
            self._persist(_AGG_PREFIX + site, json.dumps(agg, ensure_ascii=False))

    def _append_recent(self, item: dict) -> None:
        raw = self._state.get(_RECENT_KEY)
        ...
        if len(lst) > _RECENT_MAX:
            lst = lst[-_RECENT_MAX:]
        try:
            self._persist(_RECENT_KEY, lst)
        except Exception:
            self._persist(_RECENT_KEY, json.dumps(lst, ensure_ascii=False))
