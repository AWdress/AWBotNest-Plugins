...
async def _record_and_notify(ctx, store, client, message, target, site, direction,
    ...
    want_rich_table = output_mode == "rich_table"
    try:
        ...
        if sent is None and want_rich_table and entries:
            rich_parts = []
            if text:
                rich_parts.append(text)
            rich_parts.append(lb.render_rich_table(
                entries, site.site_name, site.bonus_name, direction, owner_name
            ))
            try:
                sent = await _send_rich_reply(ctx, chat_id, target, "\n".join(rich_parts))
...
async def _send_reply(client, chat_id, target, text=None, photo=None, caption=None):
    """以 Telegram HTML 回复对手方；target 为空则直接发到群。"""
    ...
    if target is not None:
        return await target.reply(text, parse_mode="html")
    return await client.send_message(chat_id, text, parse_mode="html")


async def _send_rich_reply(ctx, chat_id, target, html_text):
    """Use the platform Rich Message proxy; ordinary HTML cannot render tables."""
    user_proxy = getattr(ctx, "user", None)
    if user_proxy is None or not await user_proxy.supports_native_rich():
        raise RuntimeError("当前用户账号不支持原生 Rich Message")
    # Kurigram 2.2.24 Rich Message replies require a ReplyParameters object.
    # The plugin contract forbids importing Pyrogram types directly and the
    # platform proxy intentionally accepts no numeric reply shortcut yet, so
    # send the native table as a standalone message instead of passing the
    # unsupported reply_to_message_id keyword.
    return await user_proxy.send_rich(chat_id, html_text, format="html")
