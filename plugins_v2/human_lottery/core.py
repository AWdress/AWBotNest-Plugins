...
class LotteryManager:
    ...
    def _next_id(self) -> int:
        seq = _to_int(getattr(self, "_seq", 0), 0) + 1
        self._seq = seq
        self.ctx.create_task(self.ctx.storage.set("lottery_seq", seq), name="human-lottery-sequence")
        return seq

    async def create(self, client, message, chat, creator, params: dict) -> bool:
        key = _activity_key(client, message.chat_id)
        if key in self.active:
            await client.send_message(message.chat_id, "这个群已经有一场抽奖在进行了，先开奖或取消再创建吧～")
            return False
        ...
        activity = {
            ...
            "chat_title": getattr(chat, "title", "") or str(message.chat_id),
            "creator_id": getattr(creator, "id", 0),
            "creator_name": _display_name(creator),
            "announcement_id": announcement.id,
            "announcement_link": _message_link(chat, announcement.id),
            "prize": params["prize"],
        ...
        self.ctx.log.info(
            ...
            activity["chat_title"], message.chat_id,
    ...
    async def participate(self, client, message, user) -> bool:
        key = _activity_key(client, message.chat_id)
        activity = self.active.get(key)
        if not activity or activity["status"] != "进行中":
            return False
        if str(message.raw_text or "").strip() != activity["keyword"]:
            return False
    ...
    async def draw(self, key: str, reason: str = "手动开奖") -> bool:
        ...
        if len(participants) >= minimum:
            ...
            result_message = await activity["client"].send_message(
                ...
                reply_to=activity["announcement_id"],
                parse_mode="html",
            )
            if result_message:
    ...
    def _finish(
        ...
        self._history = history[-self.HISTORY_MAX:]
        self.ctx.create_task(self.ctx.storage.set(self.HISTORY_KEY, self._history), name="human-lottery-history")
        key = activity["key"]
...
async def setup(ctx):
    ...
    saved = await ctx.storage.get(_manager.HISTORY_KEY, [])
    _manager._history = saved if isinstance(saved, list) else []
    _manager._seq = await ctx.storage.get("lottery_seq", 0)
    ...
    async def on_owner_command(event):
        ...
        try:
            if create is not None:
                ...
                handled = True
                await _manager.status(client, event.chat_id)
            elif text == str(cfg["draw_word"]).strip():
                handled = True
                key = _activity_key(client, event.chat_id)
                if not await _manager.draw(key, reason="命令提前开奖"):
                    await client.send_message(event.chat_id, "这个群没有可以开奖的活动～")
            elif text == str(cfg["cancel_word"]).strip():
                handled = True
                key = _activity_key(client, event.chat_id)
                if not await _manager.cancel(key):
                    await client.send_message(event.chat_id, "这个群没有可以取消的抽奖～")
        except Exception as exc:  # noqa: BLE001
            ctx.log.error("处理抽奖命令失败：%r", exc)
            await client.send_message(event.chat_id, f"抽奖操作失败：{exc}")
        finally:
            if handled and cfg.get("delete_commands", True):
                await _safe_delete(message)

    @ctx.on_message(incoming=True)
    async def on_participation(event):
        if not event.is_group:
            return
        client, message = event.client, event.message
        ...
        if not ctx.config.get("enabled", DEFAULTS["enabled"]):
            return
        try:
            await _manager.participate(client, message, user)
        except Exception as exc:  # noqa: BLE001
