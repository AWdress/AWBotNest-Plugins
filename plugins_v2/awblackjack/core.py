...
async def _persist(ctx, key, value):
    _state[key] = value
    task = ctx.create_task(ctx.storage.set(key, value), name=f"AWBlackJack存储:{key}")
...
async def _read_stream(ctx, stream, level: str):
    while True:
        ...
        if not text:
            continue
        history = _state.get("recent_logs", [])
        if not isinstance(history, list):
            history = []
        await _persist(ctx, "recent_logs", [*history, text][-100:])
...
async def _handle_alert(ctx, cfg: dict, payload: dict) -> None:
    ...
    events = _state.get("alert_events", [])
    if not isinstance(events, list):
        events = []
    await _persist(ctx, "alert_events", [*events, payload][-100:])
