...
async def _fill_chat_titles(ctx, items: list[dict]) -> list[dict]:
    ...
    for item in items:
        ...
        if chat_id not in cache:
            try:
                chat = await apps[0].get_entity(chat_id)
...
async def setup(ctx):
    global _manager
    _manager = ActivityManager(ctx, dict(await ctx.storage.items()))
    ...
    async def on_create(event):
        ...
        if not params:
            return
        try:
            await _manager.create_redpacket(client, message, params, await event.get_sender(), await event.get_chat())
    ...
    @ctx.on_message(incoming=False, outgoing=True)
    async def on_status_or_end(event):
        if not event.is_group or not event.raw_text:
            return
        client, message = event.client, event.message
        if not ctx.config.get("enabled", True):
        ...
        try:
            if text == status_word:
                await _manager.get_activity_status(client, event.chat_id)
            else:
                await _manager.end_activity_by_user(client, message, await event.get_sender())
        except Exception as e:  # noqa: BLE001
    ...
    async def on_participation(event):
        ...
        if not ctx.config.get("enabled", True):
            return
        try:
            await _manager.handle_participation(client, message, await event.get_sender())
        except Exception as e:  # noqa: BLE001
