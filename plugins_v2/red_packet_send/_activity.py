...
class ActivityManager:
    ...
    def __init__(self, ctx, storage_state=None):
        self.ctx = ctx
        self.storage_state = dict(storage_state or {})
        self.storage_tasks = set()
    ...
    def _persist(self, key, value):
        self.storage_state[key] = value
        task = self.ctx.create_task(self.ctx.storage.set(key, value), name=f"red-packet-storage:{key}")
        self.storage_tasks.add(task)
        task.add_done_callback(self.storage_tasks.discard)

    def _next_id(self) -> int:
        """全局递增红包编号（持久化到 kv，重启不重复，便于后期对照）。"""
        try:
            n = int(self.storage_state.get("rp_seq", 0) or 0) + 1
        except Exception:  # noqa: BLE001
            n = 1
        try:
            self._persist("rp_seq", n)
        except Exception:  # noqa: BLE001
            pass
        return n
    ...
    # —— 创建活动 ——
    async def create_redpacket(self, client, message, params: Tuple[float, int, Optional[str]], sender=None, chat=None) -> bool:
        ...
        activity = {
            "client": client,
            "chat_id": chat_id,
            "chat_title": chat_name(chat or getattr(message, "chat", None), chat_id),
            "rp_id": rp_id,
            "creator_id": user_id,
        ...
        reply_target_id = getattr(message, "reply_to_msg_id", None)
    ...
    async def _rotate_captcha(self, client, chat_id: int, activity: Dict):
        ...
        if path:
            try:
                sent = await client.send_file(chat_id, path, caption=caption)
            except Exception as e:  # noqa: BLE001
    ...
    # —— 处理参与 ——
    async def handle_participation(self, client, message, sender=None) -> bool:
        chat_id = message.chat_id
        key = self._key(client, chat_id)
        ...
        async with self.locks[key]:
            ...
            if any(p["user_id"] == user_id for p in activity["participants"]):
                notice = await client.send_message(chat_id, "⚠️ 重复参与无效，你已经领取过这个红包了。", reply_to=message.id)
                if notice:
                    _track(self.ctx.create_task(_auto_delete(notice, 8)))
                return False
    ...
    async def _send_redpacket(self, client, message, username: str, amount: float):
        ...
        try:
            ...
            msg1 = await client.send_message(chat_id, f"{prefix}{int_amount}", reply_to=message.id)
            ...
            msg2 = await client.send_message(
                chat_id, congrats, reply_to=message.id
            )
    ...
    # —— 用户手动结束（仅创建者）——
    async def end_activity_by_user(self, client, message, sender=None) -> bool:
        chat_id = message.chat_id
        key = self._key(client, chat_id)
        activity = self.active.get(key)
        if not activity:
    ...
    async def get_activity_status(self, client, chat_id: int):
        ...
        if captcha_path and os.path.exists(captcha_path):
            ...
            try:
                await client.send_file(chat_id, captcha_path, caption=caption)
                return
            except Exception as e:  # noqa: BLE001
    ...
    def _append_history(self, activity: Dict) -> None:
        try:
            raw = self.storage_state.get(self._HISTORY_KEY, None)
            ...
            self._persist(self._HISTORY_KEY, hist)
        except Exception as e:  # noqa: BLE001
    ...
    async def close(self):
        ...
        if self.storage_tasks:
            await asyncio.gather(*list(self.storage_tasks), return_exceptions=True)
        self.storage_tasks.clear()
