...
class AWEmbyPush:
    ...
    def _save_cards(self, cards: List[dict]):
        try:
            _state_set(self.ctx, "recent_cards", json.dumps(cards[-10:], ensure_ascii=False))
        except Exception as e:
    ...
    def _send_telegram(self, media: dict):
        ...
        try:
            ...
            if result.get("ok"):
                ...
                try:
                    _state_set(self.ctx, "tg_chat_target", {"id": str(chat_id), "title": title})
...
async def setup(ctx):
    ...
    async def _api_chat_name(req):
        ...
        def _resolve():
            ...
            try:
                _state_set(ctx, "tg_chat_target", {"id": chat_id, "title": title})
            except Exception:  # noqa: BLE001
                pass
    ...
    async def _api_clear(req):
        try:
            _state_delete(ctx, "recent_cards")
        except Exception:  # noqa: BLE001
            pass
        return {"ok": True}
...
async def teardown(ctx):
    ...
    if _storage_futures:
        await asyncio.gather(
            *(asyncio.wrap_future(item) for item in list(_storage_futures)),
            return_exceptions=True,
        )
    _storage_futures.clear()
    _state.clear()
    _event_loop = None
# V2 异步存储的内存镜像。Webhook 业务在工作线程执行，写入通过主事件循环回提。
_state: dict = {}
_event_loop = None
_storage_futures: set = set()
...
def _state_set(ctx, key, value) -> None:
    ...
    if _event_loop is None:
        return
    future = asyncio.run_coroutine_threadsafe(ctx.storage.set(key, value), _event_loop)
    _storage_futures.add(future)
    future.add_done_callback(_storage_futures.discard)


def _state_delete(ctx, key) -> None:
    _state.pop(key, None)
    if _event_loop is None:
        return
    future = asyncio.run_coroutine_threadsafe(ctx.storage.delete(key), _event_loop)
    _storage_futures.add(future)
    future.add_done_callback(_storage_futures.discard)
