...
async def setup(ctx):
    ...
    saved_questions = await ctx.storage.get(_QUESTION_HISTORY_KEY, []) or []
    ...
    async def _send_next_question(client, chat_id, timeout):
        ...
        try:
            ...
            if image_path:
                try:
                    msg = await client.send_file(chat_id, image_path, caption=text)
                except Exception as exc:  # noqa: BLE001
                    ctx.log.warning("[答题] 看图题发送失败，回退封面文字题：%r", exc)
                    state["image_path"] = None
                    state["image_quiz"] = False
                    text = _question_text(state, timeout, reward)
                    msg = await client.send_file(chat_id, _QUIZ_COVER, caption=text)
            else:
                msg = await client.send_file(chat_id, _QUIZ_COVER, caption=text)
    ...
    async def _start_previous(client, chat_id, message):
        cfg = _effective_cfg(ctx)
        if chat_id in _active:
    ...
    async def _stop_previous(client, chat_id):
    ...
    async def _start(client, chat_id, message):
        ...
        try:
            ...
            await ctx.storage.set(_QUESTION_HISTORY_KEY, list(_recent_questions))
            ...
            if first.get("image_path"):
                try:
                    msg = await client.send_file(chat_id, first["image_path"], caption=text)
                except Exception as exc:  # noqa: BLE001
    ...
    async def _handle_answer(client, message, sender, chat):
        cfg = _effective_cfg(ctx)
        ...
        if text not in [correct, *aliases]:
            try:
                hint = await message.reply("❌ 答案不对，再想一想～")
                if hint:
        ...
        try:
            reward_msg = await message.reply(f"+{reward}")
            if reward_msg:
    ...
    async def on_group_message(event):
        ...
        if is_outgoing:
            return
        await _handle_answer(client, message, await event.get_sender(), await event.get_chat())
