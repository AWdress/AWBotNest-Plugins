...
__plugin__ = {
    ...
    "config_schema": {
        "auto_reminder_enabled": {"type": "boolean", "default": True, "label": "启用自动提醒", "cols": 3, "order": 1, "section": "功能开关"},
        "auto_delete_replies": {"type": "boolean", "default": True, "label": "自动删除插件回复", "cols": 3, "order": 2, "section": "功能开关"},
        "show_pet_image": {"type": "boolean", "default": True, "label": "状态时显示宠物图片", "cols": 3, "order": 3, "section": "功能开关"},
        "use_fullbody_art": {"type": "boolean", "default": True, "label": "启用全身像视觉系统", "cols": 3, "order": 4, "section": "功能开关"},
        "random_events_enabled": {"type": "boolean", "default": True, "label": "启用随机事件", "cols": 3, "order": 5, "section": "功能开关"},
        "daily_brief_enabled": {"type": "boolean", "default": True, "label": "启用周期状态播报", "cols": 3, "order": 6, "section": "功能开关"},
        "heartbeat_interval_min": {"type": "slider", "default": 60, "label": "状态检查间隔（分钟）", "min": 10, "max": 360, "step": 10, "order": 10, "section": "运行设置"},
        "decay_multiplier": {"type": "slider", "default": 100, "label": "状态衰减倍率（%）", "min": 50, "max": 300, "step": 10, "order": 11, "section": "运行设置"},
        "delete_delay_seconds": {"type": "slider", "default": 30, "label": "回复消息保留时间（秒）", "min": 5, "max": 300, "step": 5, "order": 12, "section": "运行设置"},
        "event_chance_percent": {"type": "slider", "default": 25, "label": "随机事件触发概率（%）", "min": 0, "max": 100, "step": 5, "order": 13, "section": "运行设置"},
        "daily_brief_chance_percent": {"type": "slider", "default": 20, "label": "每轮播报概率（%）", "min": 0, "max": 100, "step": 5, "order": 14, "section": "运行设置"},
        "status_cooldown_seconds": {"type": "slider", "default": 10, "label": "状态命令冷却（秒）", "min": 0, "max": 120, "step": 5, "order": 20, "section": "冷却设置"},
        "action_cooldown_seconds": {"type": "slider", "default": 20, "label": "互动命令冷却（秒）", "min": 0, "max": 180, "step": 5, "order": 21, "section": "冷却设置"},
        "use_item_cooldown_seconds": {"type": "slider", "default": 15, "label": "使用道具冷却（秒）", "min": 0, "max": 180, "step": 5, "order": 22, "section": "冷却设置"},
        "info": {"type": "info", "label": "玩法说明", "order": 30, "section": "命令说明", "text": "先发送 /领养 名字 或 .领养 名字 来领养宠物；领养后可用 /状态、/喂食、/玩耍、/清洁 与它互动。用 /档案 查看成长档案，用 /背包 查看道具，用 /使用 道具名 来使用道具。"}
    },
}
...
async def setup(ctx):
    ...
    writes: set[asyncio.Task] = set()

    def kv_set(key, value):
        state[key] = value
        task = ctx.create_task(ctx.storage.set(key, value), name=f"digital-pet-storage:{key}")
        writes.add(task)
        task.add_done_callback(writes.discard)

    def kv_delete(key):
        state.pop(key, None)
        task = ctx.create_task(ctx.storage.delete(key), name=f"digital-pet-storage-delete:{key}")
        writes.add(task)
        task.add_done_callback(writes.discard)

    def get_pet_owners():
        d = state.get("pet_owners_list")
        if not d:
            return []
        try:
            return json.loads(d)
    ...
    def add_pet_owner(uid):
        ...
        if uid not in owners:
            owners.append(uid)
            kv_set("pet_owners_list", json.dumps(owners))

    def save_pet(uid, pet: Pet):
        kv_set(f"pet_{uid}", json.dumps(pet.to_dict()))

    def get_pet(uid):
        d = state.get(f"pet_{uid}")
        if not d:
            return None
        try:
    ...
    def _check_cd(uid, name):
        now = int(time.time())
        raw = state.get(_cd_key(uid, name))
        if not raw:
            return 0
        try:
            ts = int(raw)
    ...
    def _set_cd(uid, name):
        ...
        kv_set(_cd_key(uid, name), str(expire_at))
    ...
    async def _send_photo_and_autodelete(client, chat_id, photo_path, caption):
        sent = await client.send_file(chat_id, str(photo_path), caption=caption)
        if ctx.config.get("auto_delete_replies", True):
            try:
                d = int(ctx.config.get("delete_delay_seconds", 30) or 30)
                await asyncio.sleep(d)
    ...
    async def pet_commands(event):
        ...
        async def do_status():
            ...
            if ctx.config.get("show_pet_image", True):
                try:
                    ...
                    await _send_photo_and_autodelete(client, event.chat_id, img, st)
                    try:
        ...
        async def do_profile():
            ...
            try:
                ...
                await _send_photo_and_autodelete(client, event.chat_id, img, text_out)
                try:
                    await message.delete()
                except Exception:
                    pass
        ...
        async def do_interact(action):
            ...
            if preferred:
                try:
                    await _send_photo_and_autodelete(client, event.chat_id, base_dir / preferred, f"{reply_text} (XP +{xp_gain}){extra}")
                    try:
                        await message.delete()
                    except Exception:
                        pass
