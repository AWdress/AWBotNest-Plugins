...
async def setup(ctx):
    ...
    async def on_new_lottery(event):
        ...
        # ── 奖品过滤（默认除陷阱外全部参与；勾选「单独匹配」才按 PRIZE_LIST 白名单过滤）──
        # 单独匹配：把 PRIZE_LIST 当自定义奖品名白名单，跨所有行匹配关键词、忽略群组列。
        matched_group = event.chat_id
        if cfg.get("only_listed_prizes", False):
            prize_map = parse_prize_list(cfg.get("prize_list", ""))
            hit = match_prize_group(
    ...
    async def _participate(client, message, chat, lottery_id, info):
        ...
        if cfg.get("lottery_wait_enabled", False):
            ...
            if message.chat_id in overrides:
                wmin, wmax = overrides[message.chat_id]
            else:
    ...
    # ============================================================
    @ctx.on_message(incoming=True)
    async def on_capture_first(event):
        client, message = event.client, event.message
        sender = await event.get_sender()
        if message.out or getattr(sender, "bot", False):
            return
        try:
            ...
            for lottery_id, data in waiting:
                ...
                if not keyword or keyword_clean not in (message.text or ""):
                    continue
                if target_chat_id and event.chat_id != target_chat_id:
                    continue
            ...
            if my_id and my_id in winner_block:
                ...
                if cfg.get("lottery_thank_message", False):
                    ...
                    if texts:
                        ...
                        try:
                            await client.send_message(message.chat_id, line)
                        except Exception:  # noqa: BLE001
                            pass
                # 回复用户名（有转账功能的群跳过）
                if cfg.get("username_reply_switch", False) and message.chat_id not in transfer_groups:
                    pt = cfg.get("auto_lottery_username", "")
                    if pt:
                        try:
                            await client.send_message(
                                message.chat_id, f"{boss_name}大佬，我的是: {pt}")
                        except Exception:  # noqa: BLE001
                            pass
            else:
                # 自己参与但没中奖 → 黑幕
                if entry.get('flag') == 1 and cfg.get("lottery_heimu_message", False):
                    ...
                    if random() > 0.2:
                        ...
                        if texts:
                            try:
                                await client.send_message(
                                    message.chat_id, texts[randint(0, len(texts) - 1)])
                            except Exception:  # noqa: BLE001
                                pass
    ...
    # 4. 负面回复（被质疑是机器人）
    # ============================================================
    @ctx.on_message(pattern=r"机器人|真人？|脚本|自动抽奖|不是真人|脚本抽奖|机器人抽奖|这个也是", incoming=True)
    async def on_negative_reply(event):
        client, message = event.client, event.message
        cfg = ctx.config
        if not cfg.get("lose_reply_switch", False):
            return
        # 必须是回复自己的消息
        rtm = await message.get_reply_message()
        if not (rtm and rtm.out):
            return
    ...
    # ============================================================
    @ctx.on_message(pattern=r"^(?:/sendprize|\.sendprize)\b", incoming=False, outgoing=True)
    ...
    @ctx.on_message(pattern=r"^(?:/listprize|\.listprize)\b", incoming=False, outgoing=True)
    async def cmd_listprize(event):
        client, message = event.client, event.message
        pending = _store.all()
        if not pending:
        ...
        for client in apps:
            try:
                async for d in client.iter_dialogs():
                    ...
                    if not chat:
                        continue
                    if not (getattr(chat, "megagroup", False) or chat.__class__.__name__ == "Chat"):
                        continue
                    if chat.id in seen:
                        continue
                    seen.add(chat.id)
