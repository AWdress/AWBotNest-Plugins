    ...
    "config_schema": {
        ...
        "rules_text": {
            ...
            "fields": {
                ...
                "extra_reply": {"type": "string", "label": "追加回复内容", "default": "叮！恭喜你喜提特等奖掉落。"},
                "cooldown_notify": {
        ...
        for d in raw:
            if isinstance(d, dict):
                ...
                if reply:
                    ...
                    extra_reply = str(d.get("extra_reply", "叮！恭喜你喜提特等奖掉落。") or "").strip()
    ...
    async def keyword_listener(client, message):
        ...
        try:
            for keyword, reply, cooldown_notify, match_type, cooldown_hours, trigger_mode, reset_at_midnight, count_for_leaderboard, fun_reply_chance, fun_replies, trigger_chance, extra_reply_enabled, extra_reply in rules:
                ...
                if extra_reply_enabled and extra_reply:
                    extra_sent = await client.send_message(
                        chat_id, _render(extra_reply, message), reply_to_message_id=message.id
                    )
