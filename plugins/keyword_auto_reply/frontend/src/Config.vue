        ...
        <ol v-else class="rule-list">
          <li v-for="(rule, index) in cfg.rules_text" :key="index" :class="{ open: openRule === index }">
            ...
            <div v-if="openRule === index" class="editor">
              ...
              <label v-if="rule.extra_reply_enabled"><span>追加回复内容</span><textarea v-model="rule.extra_reply" rows="3" placeholder="叮！恭喜你喜提特等奖掉落。掉落 {number} 茉莉"></textarea><small class="field-help">仅标准回复命中时发送，趣味文字不会追加。{number} 会复用标准回复中第一个随机数。</small></label>
