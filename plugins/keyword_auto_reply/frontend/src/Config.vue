        ...
        <ol v-else class="rule-list">
          <li v-for="(rule, index) in cfg.rules_text" :key="index" :class="{ open: openRule === index }">
            ...
            <div v-if="openRule === index" class="editor">
              ...
              <div class="field-grid"><label><span>趣味文字概率（%）</span><input v-model.number="rule.fun_reply_chance" type="number" min="0" max="100" step="1"><small class="field-help">设为 0 表示始终发送标准回复。</small></label><label><span>趣味文字（每行一条）</span><textarea v-model="rule.fun_replies" rows="3" placeholder="今天先放你一马～&#10;这次羊毛变成空气啦！"></textarea></label></div>
