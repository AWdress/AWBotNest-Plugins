...
def _target_id(cfg):
    if _topic_mode(cfg) == "bot":
        raw_admins = str(cfg.get("admin_ids", "")).replace("，", ",")
    ...
    if not force and existing and existing.get("topic_id"):
        if mode == "bot":
            try:
                ...
                # 查询失败时绝不清映射或新建，避免网络波动制造重复话题。
                raise RuntimeError(f"无法核验已有私聊话题 {existing.get('topic_id')}：{exc}") from exc
            current = next(
                (item for item in matches if int(getattr(item, "id", 0) or 0) == int(existing["topic_id"])),
                None,
            )
            if current:
            ...
            if matches:
                ...
                old_topic_id = existing.get("topic_id")
                existing["topic_id"] = int(chosen.id)
                existing["target_id"] = target_id
                existing["topic_mode"] = mode
                topics[key] = existing
                _set_dict(ctx, "topics", topics)
                ctx.log.info("私聊话题 %s 已失效，改为复用同用户话题 %s", old_topic_id, chosen.id)
                return int(chosen.id)
            ctx.log.warning(
                "用户 %s 的私聊话题 %s 已删除或关闭，将重新创建",
...
async def _copy_messages_to_topic(client, target_id, topic_id, messages, mode):
    """由 Telegram 服务端无署名复制媒体，避免转发来源的 file_id 不可复用。"""
    source_chat_id = messages[0].chat.id
    private_topic = mode == "bot"
    result = await client.invoke(
        raw.functions.messages.ForwardMessages(
            ...
            drop_author=True,
            top_msg_id=None if private_topic else topic_id,
            reply_to=(raw.types.InputReplyToMessage(reply_to_msg_id=topic_id)
                      if private_topic else None),
        )
    )
    ...
    # 私聊话题的返回消息可能省略 reply_to，0 不代表消息落入“全部”；只有返回了
    # 非零且不同的路由 ID 才判错。群组论坛仍保持严格校验。
    route_mismatch = (
        any(routed_id != int(topic_id) for routed_id in routed_ids)
        if not private_topic else
        any(routed_id not in (0, int(topic_id)) for routed_id in routed_ids)
    )
    if raw_messages and route_mismatch:
        if sent_ids:
            try:
...
async def _send_content_to_topic(client, target_id, topic_id, message, mode):
    """等价于原项目 copy_message：所有内容均由 Telegram 服务端无署名复制。"""
    sent_ids = await _copy_messages_to_topic(client, target_id, topic_id, [message], mode)
    if sent_ids:
        return sent_ids[0]
    if mode == "bot":
        # Bot 私聊话题的 Updates 可能不包含可解析消息 ID；RPC 无异常即代表已接收。
        return 0
...
async def _forward_one_unlocked(ctx, client, message, cfg):
    ...
    mode = _topic_mode(cfg)
    try:
        sent_id = await _send_content_to_topic(client, target_id, topic_id, message, mode)
    except Exception as exc:
        if not _missing_topic_error(exc):
            raise
        topics = _topics(ctx)
...
async def _forward_one(ctx, client, message, cfg):
    ...
    async with _topic_locks[int(message.from_user.id)]:
        await _forward_one_unlocked(ctx, client, message, cfg)
...
async def setup(ctx):
    ...
    if cfg_at_start.get("enabled") and _target_id(cfg_at_start) and ctx.bot.connected:
        try:
            raw_bot = ctx.bot.raw
    ...
    async def api_status(req):
        ...
        return {"bot_running": bool(cfg["enabled"]), "bot_status": "运行中" if cfg["enabled"] else "已停止",
                "group_title": str(_target_id(cfg) or "-"), "topic_mode": _topic_mode(cfg),
    ...
    async def private_message(client, message):
        ...
        if not cfg["enabled"] or not message.from_user:
            return
        if (_topic_mode(cfg) == "bot" and _target_id(cfg)
